...
import { FaStar, FaEdit, FaTrash, FaCheck, FaBan, FaSpinner, FaExclamationTriangle } from 'react-icons/fa';
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success && Array.isArray(data.data)) {
          ...
          if (data.error && data.error.includes('requires an index')) {
            setIndexError(true);
          }
        }
      } else {
        ...
        try {
          ...
          if (errorData.error && errorData.error.includes('requires an index')) {
            setIndexError(true);
          }
        } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="max-w-7xl mx-auto">
        {indexError && (
          <div className="mb-6 p-4 bg-yellow-100 border border-yellow-400 text-yellow-800 rounded flex items-start">
            <FaExclamationTriangle className="text-yellow-600 mt-1 mr-3 flex-shrink-0" />
            <div>
              <p className="font-medium">Firestore Index Required</p>
              <p className="mt-1">A Firestore index is required for this query. You need to create a composite index for the testimonials collection.</p>
              
              <div className="mt-3">
                <Link 
                  href="/admin-dashboard/testimonials/fix-index" 
